...
airflow dags trigger churn_pilot_pipeline

# Run specific task
airflow tasks run churn_pilot_pipeline data_ingestion 2024-01-01

# Test single task (dry run)
airflow tasks test churn_pilot_pipeline data_ingestion 2024-01-01
```
...
# Show DAG details
airflow dags show churn_pilot_pipeline

# List DAG runs
airflow dags list-runs -d churn_pilot_pipeline

# Show task instances for a run
airflow tasks list churn_pilot_pipeline --tree
```

### View Logs
```bash
# View task logs
airflow tasks logs churn_pilot_pipeline data_ingestion 2024-01-01 1

# View all logs for a DAG run
airflow dags logs churn_pilot_pipeline
```
...
```bash
# Check DAG state
airflow dags state churn_pilot_pipeline 2024-01-01

# List failed tasks
airflow tasks failed-deps churn_pilot_pipeline 2024-01-01

# Clear task state (to rerun)
airflow tasks clear churn_pilot_pipeline

# Pause/Unpause DAG
airflow dags pause churn_pilot_pipeline
airflow dags unpause churn_pilot_pipeline
```

### 3. Pipeline Health Checks
```bash
# Validate DAG structure
airflow dags check churn_pilot_pipeline

# Test DAG import
python -c "from airflow.models import DagBag; print(DagBag().get_dag('churn_pilot_pipeline'))"
...
airflow tasks clear churn_pilot_pipeline -t task_name
...
# Permission issues
chmod +x airflow/dags/churn_pilot_pipeline.py
```
...
To modify the schedule, edit `airflow/dags/churn_pilot_pipeline.py`:
```python
...
├── processed/training_sets/churn_pilot_v1_YYYYMMDD_HHMMSS.csv
...
airflow dags trigger churn_pilot_pipeline && \
airflow dags list-runs -d churn_pilot_pipeline --limit 1
```
...
airflow tasks test churn_pilot_pipeline data_ingestion 2024-01-01
airflow tasks test churn_pilot_pipeline raw_data_storage 2024-01-01
airflow tasks test churn_pilot_pipeline data_validation 2024-01-01
airflow tasks test churn_pilot_pipeline data_preparation 2024-01-01
airflow tasks test churn_pilot_pipeline data_transformation 2024-01-01
airflow tasks test churn_pilot_pipeline feature_store 2024-01-01
airflow tasks test churn_pilot_pipeline data_versioning 2024-01-01
airflow tasks test churn_pilot_pipeline model_building 2024-01-01
```
