# ChurnPilot — Steer Customer Retention with Data-Driven ML

**An end-to-end ML pipeline that turns raw telecom customer data into churn predictions you can act on.**

ChurnPilot automates the full journey from ingestion to deployment — collection, validation, feature engineering, model training, and versioning — wrapped in consistent logging and reproducibility at every stage. It now also ships a Databricks migration path (`databricks/`) that re-platforms the same pipeline onto Unity Catalog, Delta Live Tables, and Asset Bundle Workflows.

## Why ChurnPilot

Predicting churn early is what lets a telecom act before a customer walks. ChurnPilot exists to make that prediction pipeline **reproducible, observable, and easy to operate** — not a one-off notebook, but a pipeline that logs every run, versions every dataset, and tracks every model.
...
├── databricks/                    # Databricks migration (Unity Catalog, DLT, Asset Bundles)
├── docs/                          # Documentation
├── logs/                          # Pipeline logs
